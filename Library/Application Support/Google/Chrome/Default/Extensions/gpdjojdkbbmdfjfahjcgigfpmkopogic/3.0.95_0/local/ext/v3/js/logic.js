// add anon flag to $.f.log so we can hide xuid if needed
// rename $.a.noPinList to $.a.nopeList; don't log pfob if we're on domains included in $.a.nopeList

// our logic version
const VERSION = 2018051501;
    ...
    f: {
      ...
      over: e => {
        ...
        if (el && el.tagName) {
          if (el === $.s.buttonSave || el === $.s.buttonSearch) {
            ...
            if (el.tagName === 'IMG') {
              if (!$.v.hazLogin && !el.src.match(/^((http|https):\/\/)/)) {
                $.s.buttonSave.style.display = 'none';
              } else {
                if (!$.v.nopin) {
      ...
      check: () => {
        ...
        f = [
          ...
          () => {
            let patch, mod = {}, arr = [], obj = {}, meta = $.d.getElementsByTagName('META'), key, value, i, j, p, q, z, lkey, lval;
